  ...
  try {
      ...
      for (const [index, element] of data.entries()) {         
          if (element[0] != undefined) {
              ...
              var nangluclaprap = parseInt(element[3], 10);
            //   if (element[3]) {
            //     var nangluclaprap = nangluclaprap.substring(0, 5);               
            //   }
              ...
              for (var i = 8; i < 18; i++) {
                  if (element[i] != undefined && element[i] != '' && index != 0) {
                      ...
                      if (results && results.length > 0) {
                          await queryMySQL(sql_update, [element[i], nangluclaprap, results[0].id]);
                      } else {
      ...
      parts.forEach((part) => {
        ...
        if (date_sls.length === 2) {        
          if (date_sls[0] == ngay_check ) {
            console.log(item.line, giobatdau, ngay_check);
            console.log("dô");
            const [giobatdauHour, giobatdauMinute] = giobatdau
              .split(":")
              .map(Number);
            ...
            if (giotdHour < 6) {
              ...
              if ((t2 || t3) && item.check_chunhat == "K") {
                let ngay_sub = new Date(
                  date_sub.setDate(date_sub.getDate() - 2)
                );
                if (item.line.startsWith("LR5")) {
            ...
            console.log("nllr" + nllr);
            if (nllr) {
              const tg_tmp = Math.round((date_sls[1] / (nllr / 8)) * 100) / 100;
              console.log("tính năng lực láp ráp" + tg_tmp, date_sls[1], nllr);
              ...
              thoi_gian_lap_rap.setMinutes(
                thoi_gian_lap_rap.getMinutes() + phut
              );
              ...
              console.log("giobatdau" + thoi_gian_lap_rap);
            }
          } else {
            data.push({
              ngay: date_sls[0],
...
function formatNgay(ngay, gio) {
  ...
  //     `${(roundedHours % 12 || 12).toString().padStart(2, '0')}:${roundedMinutes
  //         .toString()
  //         .padStart(2, '0')}:00 ` +
